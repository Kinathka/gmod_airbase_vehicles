...
function ENT:CreateRotors()
	self.Rotors.Front = ents.Create( "avehicle_blackhawk_rotor" )
	--self.Rotors.Front:SetModel(self.Models.RotorBase)
	self.Rotors.Front:SetName("Heli Blackhawk Rotor")
	self.Rotors.Front:SetOwner(self.Owner)
	self.Rotors.Front.Heli = self
	self.Rotors.Front:SetAngles(self.Entity:GetAngles())
	self.Rotors.Front:SetPos(self.Entity:GetPos() + (self.Entity:GetUp() * 130) + (self.Entity:GetForward() * 0)) 
	self.Rotors.Front:Spawn()

	self.Rotors.Back = ents.Create("prop_physics")
	self.Rotors.Back:SetModel(self.Models.RotorTail)
	self.Rotors.Back:SetName("Heli Blackhawk Back Rotor")
	self.Rotors.Back:SetAngles(self.Entity:GetAngles())
	self.Rotors.Back:SetPos(self.Entity:GetPos() + (self.Entity:GetUp() * 154)  + (self.Entity:GetForward() * -397) + (self.Entity:GetRight() * 24)) --330.2450 6.6013 135.5856
	...
	--Constraint prop
	constraint.Axis( self.Entity, self.Rotors.Front, 0, 0, Vector(0,0,125), frotorOffset , 0, 0, 1, 1)	
	constraint.Axis( self.Entity, self.Rotors.Back, 0, 0, Vector(-397,28,152) , Vector(0,0,0), 0, 0, 1, 1)
