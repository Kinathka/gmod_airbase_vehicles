...
function ENT:Initialize()
	...
	self.Rotors.FrontSpeed = 165
	self.Rotors.BackSpeed = 165
...
function ENT:CreateRotors()
	self.Rotors.Front = ents.Create( "avehicle_ch46_rotor" )
	self.Rotors.Front:SetName("CH46 Heli Rotor")
	self.Rotors.Front:SetOwner(self.Owner)
	self.Rotors.Front.Heli = self
	self.Rotors.Front:SetAngles(self.Entity:GetAngles())
	self.Rotors.Front:SetPos(self.Entity:GetPos() +  (self.Entity:GetForward() * 149) + (self.Entity:GetUp() * 170))
	self.Rotors.Front.IsAVehicleObject = true
	self.Rotors.Front:Spawn()
	--149.9105 0.1217 176.0312
	---263.2999 -0.6672 208.0313
	self.Rotors.Back = ents.Create("avehicle_ch46_rotor")
	self.Rotors.Back:SetName("CH46 Heli Rotor")
	self.Rotors.Back:SetOwner(self.Owner)
	self.Rotors.Back.Heli = self
	local backpos = self.Entity:GetPos() + (self.Entity:GetForward() * -263) + (self.Entity:GetUp() * 202)
	--local dif = (self.Entity:GetPos() + (self.Entity:GetForward() * -250) + (self.Entity:GetUp() * 208)) - backpos
	self.Rotors.Back:SetAngles(self.Entity:GetAngles() )
	self.Rotors.Back:SetPos(backpos)
	self.Rotors.Back:Spawn()
	self.Rotors.Back.IsAVehicleObject = true
	
	/* Something wrong with physics because this doesn't work as expected.
	--Front rotor center of mass.
	...
	--Back rotor center of mass.
	local brphys = self.Rotors.Back:GetPhysicsObject()
	local brotorOffset = Vector(0,0,0)
	if brphys and brphys:IsValid() then
		brotorOffset = brphys:GetMassCenter()
	end
	*/
	
	
	--Constraint prop
	constraint.Axis( self.Entity, self.Rotors.Front, 0, 0, Vector(149,0,174), Vector(0,0,0), 0, 0, 1, 1)	
	constraint.Axis( self.Entity, self.Rotors.Back, 0, 0, Vector(-263,0,206) , Vector(0,0,0), 0, 0, 1, 1)
...
function ENT:PropMotion()
	...
	if IsValid(self.Rotors.Back) then
		...
		if phys:IsValid() then
			phys:AddAngleVelocity(Vector(0,0, (self.Rotors.BackSpeed)) )
		end
	end
end
...
function ENT:StartupMotion()
	...
	if IsValid(self.Rotors.Back) then
		local phys = self.Rotors.Back:GetPhysicsObject()
		if phys:IsValid() then
			phys:AddAngleVelocity(Vector(0,0, (self.Rotors.BackSpeed * 0.1)) )
		end
	end
end
