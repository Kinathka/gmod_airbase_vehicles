...
language.Add("avehicle_heli_huey","Heli CH46")
...
function ENT:ClViewCalcFP(origin, angles, fovv)
	...
	if AVehicles.Vehicle and AVehicles.Vehicle.IsIn  and IsValid(AVehicles.Vehicle.Ent) then
		...
		local pos = ent:GetPos()+(ent:GetUp()*110)+(ent:GetForward()*182)+(ent:GetRight()*20)
...
function ENT:DrawEngineEffect()
	...
	if (self.Emitter) then
		...
		local pos = self.Entity:GetPos() +(self.Entity:GetForward() * -328) + (self.Entity:GetUp() * 111) 
