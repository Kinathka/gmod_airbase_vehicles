...
function ENT:SetupRotorEngine()
	...
	self.RotorEngine = {}
	self.RotorEngine.SlowDownSpeed = 1.2
	self.RotorEngine.BalancePitchAdd = 3.0
	self.RotorEngine.BalanceRollAdd = 3.0
	self.RotorEngine.PitchSpeedMul = 0.2
	self.RotorEngine.RollSpeedMul = 0.2
	self.RotorEngine.AutoRollCounter = 0
	self.RotorEngine.MaxSideSpeed = 650
	self.RotorEngine.MaxFWDSpeed = 750
	self.RotorEngine.levelout = false
	self.RotorEngine.hovermode = true
	self.RotorToEngineSpeed = 300 --The speed that the main rotor must turn for the engine to work.
end
