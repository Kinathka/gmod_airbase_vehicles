...
language.Add("avehicle_heli_huey","Huey Uh-1H")
...
function ENT:ClViewCalcFP(origin, angles, fovv)
	...
	if AVehicles.Vehicle and AVehicles.Vehicle.IsIn  and IsValid(AVehicles.Vehicle.Ent) then
		...
		local pos = ent:GetPos()+(ent:GetUp()*80)+(ent:GetForward()*90)+(ent:GetRight()*20)
...
function ENT:DrawEngineEffect()
	...
	local pos = self.Entity:GetPos() +(self.Entity:GetForward() * -130) + (self.Entity:GetUp() * 120) 
...
function ENT:SoundsEnginePitchCall(active)
	if self.Sounds.Enabled and active then
		...
		self.Sound_Handlers.EngineLoop:ChangePitch(tonumber(EnginePitch + VelocityPitch + doppler_effect), 0.1)
