...
function EFFECT:Init(data)
	...
	self.Position2 = self.Position + self.Scale*16*self.Normal
	...
	--dust puff
		for i=1,math.ceil(self.Scale*12) do
			...
			local velocity = math.Rand(1000,2000)*vecang
	...
	--dust cloud
		for i=1,math.ceil(self.Scale*12) do
			...
			local velocity = math.Rand(150,200)*vecang*self.Scale
			local particle = emitter:Add("particle/particle_smokegrenade", self.Position + vecang*math.Rand(20,80)*self.Scale)
			local dietime = math.Rand(2.9,2.5)*self.Scale
			particle:SetVelocity(velocity)
			particle:SetGravity(Vector(0,0,math.Rand(250,250)))
			...
			--pillar of dust
		for i=1,math.ceil(self.Scale*46) do
			...
			local dietime = math.Rand(2.5,2.9)*self.Scale
			particle:SetVelocity(velocity)
			particle:SetGravity(Vector(0,0,math.Rand(0,250)))
			particle:SetAirResistance(65)
			particle:SetDieTime(dietime)
			...
			particle:SetThinkFunction(SmokeParticleUpdate)
			particle:SetNextThink(CurrentTime + 0.5*dietime)
			particle:SetStartSize(math.Rand(50,50)*self.ScaleSlow)
			particle:SetEndSize(math.Rand(50,50)*self.ScaleSlow)
	...
	--dirt
		for i=1,math.ceil(math.Rand(9,13)*self.ScaleSlow) do
			...
			particle:SetColor(Color(173,160,143,255))
